...
/**
 * `gui.yml`, read as text, for the two files that hold promises kept in it.
 *
 ...
 *
 * At the root of the tree gui/ lives in (RK1697), under the name `gui.yml`: GitHub runs only
 * the workflows there, so the `ci.yml` this app carried as its own repository moved up.
 */
export const WORKFLOW: string = readFileSync(
  path.join(REPO, '..', '.github', 'workflows', 'gui.yml'),
  'utf8',
)
