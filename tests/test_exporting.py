...
- **No web UI** — the store is the repository.
- **No model** — the tool validates and renders.

## Done when — Block A

- **The parse round-trips** — every source line comes back byte-identical.

## Done when — RK1

- **The first symptom stops** — measured on this repository's own docs.
"""

SPEC_PROSE = """# Improvements

## Block A — The model (a task is data first)

### §RK1 The first design

The paragraph a reviewer needs, which is the one thing a bounded read elides first and
the one thing a spec exists to hand over whole.
"""


SPEC_CONFIG = """prefix = "RK"
...
def _spec_project(tmp_path: Path) -> Config:
    (tmp_path / "roadkeep.toml").write_text(SPEC_CONFIG, encoding="utf-8")
    for name, body in {
        "ROADMAP.md": SPEC_ROADMAP,
        "CHANGELOG.md": LEDGER,
        "IMPROVEMENTS.md": SPEC_PROSE,
    }.items():
        with (tmp_path / name).open("w", encoding="utf-8", newline="") as handle:
            handle.write(body)
    return Config.discover(tmp_path)
...
def test_the_spec_joins_the_four_stores_into_one_document(tmp_path):
    """The defect. `brief` composes exactly this and is bounded to a tool result, so a
    reviewer on a pull request, a second agent on a branch and a CI job asserting the
    criteria each need the join and none of them can run the read."""
    written = exporting.spec(_spec_project(tmp_path), "RK1").markdown()

    assert written.startswith("# RK1 — A first symptom")
    # Each store gets its own heading, so a reviewer can tell which file a sentence is from.
    for heading in ("## Why", "## Done when — RK1", "## Non-goals", "## Design"):
        assert heading in written, heading
    # The design entire, which is the half a result-sized read elides first.
    assert "### §RK1 The first design" in written
    assert "the one thing a spec exists to hand over whole." in written
    # Derived and stamped with nothing, so a re-run with nothing to say makes no diff.
    assert "generated by `roadkeep export --spec`" in written
    assert written == exporting.spec(Config.discover(tmp_path), "RK1").markdown()


def test_the_spec_carries_a_list_the_bounded_read_would_have_cut(tmp_path):
    # The whole difference from `brief`: a lead is uncut and a list is entire, because the
    # ceiling here is a file's rather than a tool result's.
    from roadkeep.briefing import brief

    config = _spec_project(tmp_path)
    written = exporting.spec(config, "RK1").markdown()
    for lead in brief(config, "RK1").non_goals.leads:
        assert lead.rstrip(".") in written or lead in written


def test_the_spec_is_a_subject_and_never_a_third_destination(tmp_path, capsys):
    # `--readme` and `--site` are two destinations of one projection about the whole backlog;
    # this answers about one line, so composing them would splice two documents into one file.
    _spec_project(tmp_path)
    assert main(["-C", str(tmp_path), "export", "--spec", "RK1", "--json"]) == EXIT_USAGE
    assert "one answer per call" in capsys.readouterr().err

    assert main(["-C", str(tmp_path), "export", "--spec", "RK1"]) == EXIT_OK
    assert "# RK1 — A first symptom" in capsys.readouterr().out


def test_an_id_no_file_carries_is_refused_and_names_both(tmp_path, capsys):
    _spec_project(tmp_path)
    assert main(["-C", str(tmp_path), "export", "--spec", "RK99"]) == EXIT_USAGE
