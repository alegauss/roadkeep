...
# -- what a commit carries that is not the work (RK1686) ----------------------


def _generating(tmp_path: Path, declared: str) -> Config:
    """A project whose commits carry generated art beside the work, as Cottony's do."""
    ...
    # Committed here, so the ship below is the only commit the weight is taken over: a config
    # edit riding along would be counted as the work, which is the thing being measured.
    git_commit(tmp_path, "chore: declare what a commit carries")
    return Config.discover(tmp_path)


def _with_sprites(config: Config, task_id: str, sprites: int = 12) -> None:
    """Ship one task in a commit that also regenerates `sprites` art files."""
    art = config.root / "art"
    art.mkdir(exist_ok=True)
    for n in range(sprites):
        (art / f"sprite_{n}.import").write_text(f"generated {n}\n", encoding="utf-8")
    ship(config, task_id, weight=3)


def test_the_generated_half_of_a_commit_is_left_out_of_both_axes(tmp_path):
    """RK1686. Measured on Cottony, a Godot match-3 whose sprites are generated by a script
    and committed on purpose: one change to `specials.py`, and the commit `weight` reads
    carries 46 files and 770 lines against 8 files and 78 authored.

    Both axes wrong by roughly six, and wrong in one direction — the blocks that generate most
    price as the most expensive, which is the opposite of what an agent pays. The docstring
    calls files *what an agent holds in context*, and an agent holds none of those."""
    config = _generating(tmp_path, "art/")
    _with_sprites(config, "RK1", sprites=12)
    (weighed,) = weigh(config).weighed
    # The authored half: the ledger entry and the one source file, never the twelve sprites.
    assert weighed.files == 2, weighed
    assert weighed.carried == 12, weighed


def test_what_was_left_out_is_stated_rather_than_silently_subtracted(tmp_path, capsys):
    """The property a derived answer rests on (RK71): a reader who doubts the number checks it
    against `git show`. A size smaller than the commit's own with nothing saying why is a
    figure they have to take on trust, which is what the derivation exists not to ask for."""
    config = _generating(tmp_path, "art/")
    _with_sprites(config, "RK1", sprites=12)
    assert main(["-C", str(config.root), "weight"]) == EXIT_OK
    assert "carried  12 file(s)" in capsys.readouterr().out

    assert main(["-C", str(config.root), "weight", "--records", "--json"]) == EXIT_OK
    answer = json.loads(capsys.readouterr().out)
    assert answer["carried"] == 12
    assert answer["weighed"][0]["carried"] == 12


def test_a_project_declaring_nothing_is_weighed_exactly_as_before(tmp_path):
    """The filter only ever removes, and it removes nothing where nothing is declared — which
    is every project until one looks."""
    config = repo(tmp_path)
    _with_sprites(config, "RK1", sprites=12)
    (weighed,) = weigh(config).weighed
    assert weighed.carried == 0
    # The sprites counted, because this project never said they were not the work.
    assert weighed.files == 14, weighed


def test_a_directory_is_named_with_a_slash_and_a_glob_is_still_refused(tmp_path):
    """RK1686 against RK1496. The no-globs rule stands: what changed is that a generator's
    output is named by the generator and grows, so a project names the directory it owns
    rather than listing 74 paths — and `*` is still a shape guessed at rather than a place."""
    from roadkeep.config import ConfigError, incidental_for

    config = _generating(tmp_path, "art/")
    assert config.incidental == ("art/",)
    assert incidental_for(config.incidental, "art/sprite_0.import") == "art/"
    # Exactly, which is what every project declaring a hook's three files already relies on.
    assert incidental_for(("a/b.py",), "a/b.py") == "a/b.py"
    assert incidental_for(("art/",), "artwork/x.png") == ""
    assert incidental_for(("a/b.py",), "a/b.pyc") == ""

    with pytest.raises(ConfigError) as raised:
        _generating(tmp_path / "glob", "art/**")
    assert "is a pattern" in "\n".join(raised.value.problems)
