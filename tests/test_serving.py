...
# -- the figures a decision rests on (RK1588, swept in test_figures since RK1639) ----------
#
# The sweep moved rather than being copied. RK1588 held it over the withholding reasons alone;
# RK1639 measured that `remedying`'s remedies, `serving`'s notes and `guarding`'s refusals are
# the same shape unread — each a closed set, each reaching a **caller** rather than a
# maintainer — and one reading over four tables is the alternative to four readings that agree
# until somebody edits one. `tests/test_figures.py` is the census and `surface.measured` is
# the one line that tells a measurement from an address.
