...
#: A command as a declarative file spells one: the value of `run:` or `entry:`, and only
#: when it starts the tool. Anchored on the key so that prose in a comment — this file's
#: own examples included — is never mistaken for a declaration.
...
def test_every_surface_declares_the_command_the_cli_accepts():
    # The whole point: three files that say `roadkeep lint`, and one parser that decides
...
def test_the_surfaces_are_valid_yaml():
    ...
    assert set(action["inputs"]) == {"directory", "python-version"}
