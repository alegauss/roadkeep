...
### §RK1687 A number to read, or a command to run

The alternative was a declared **command** the project owns — `saturation.py board.png
:: >= 0.88` — which reads the picture properly and answers anything. It was rejected:
this tool runs in CI, as a pre-commit hook and as a plugin the agent boundary trusts,
and a governed Markdown file that executes commands is a different tool. A query would
also stop being free.

What is left needs no execution: the project's own tooling already computes the number
and writes it somewhere, and this reads it.

Weighed against "no model and no prompts", which binds hardest here and is not
approached: the comparison relates two numbers, one from a file and one from the clause,
...
- ✅ **RK1698** **With gui/ in the tree, lint, the guard hook and MCP started at the root govern RK only, so gui/docs goes unguarded** — The root session's MCP server answers for the root only; a nested file is sent to its own shell, since a directory on every tool would outgrow the tool budget.
- ✅ **RK1699** **The plugin's source is the repository root, so installing it would copy the Electron app into every adopter's cache** — The plugin lives in plugin/ with src/ inside it: a relative source copies its directory whole and may read nothing above it, so a CI-built payload branch was the other door.
- ✅ **RK1704** **The reader's public site builds here but deploys nowhere, since this repository's one Pages site is roadkeep's** — The reader's site is a path of roadkeep's one Pages deploy and not its own: keeping the archived repository publishing would need a build fed to it from here.
- ✅ **RK1702** **A release tag here publishes the CLI and builds no installer, so the app ships from a different commit** — The app carries roadkeep's version, stamped on every commit, so one tag names both; its own number would have titled installers the release disagrees with.
