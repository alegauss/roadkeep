...
- 📋 **RK1194** (deps: —) **A local variable with an import's name makes the unspelled-import test read a dead import as used** — The scan has no scopes, so a local that is read counts as a module-level use — the one-clause fix changes nothing, and the crude one reports 32, most of them false. → §RK1194
