...
- 📋 **RK397** (deps: —) **A prose paragraph whose line starts with a bold span is inserted verbatim instead of filled** — structural asks whether any line opens with a list marker and a ** delimiter answers yes, so the paragraph keeps the author's breaks and nothing gates the width. → §RK397
