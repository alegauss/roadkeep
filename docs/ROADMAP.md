...
- 💭 **RK1699** (deps: RK1697 ✅) (requires: decision) **The plugin's source is the repository root, so installing it would copy the Electron app into every adopter's cache** — An adopter who wanted only the hook and the MCP tools should not pay for a desktop app's sources and lockfile. → §RK1699
...
- 📋 **RK1704** (deps: RK1697 ✅) (requires: decision) **The reader's public site builds here but deploys nowhere, since this repository's one Pages site is roadkeep's** — Moving the app in kept the site's build gate and dropped its deploy, so the page at alegauss.github.io/roadkeep-gui/ stops changing. → §RK1704
