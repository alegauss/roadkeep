...
- 📋 **RK375** (deps: —) **A pyproject that configures roadkeep measures as an empty backlog, the reading RK374 refused for the other declaration** — The guard is by filename so it cannot reach one, while `_configured` already answers whether a pyproject declares this tool and the refusal never asks it. → §RK375
