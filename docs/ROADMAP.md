...
- 📋 **RK1194** (deps: —) **A local variable with an import's name makes the unspelled-import test read a dead import as used** — The scan has no scopes, so a local that is read counts as a module-level use — three candidate fixes are measured out, and the stdlib's own is blind to annotations. → §RK1194
