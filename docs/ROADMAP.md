...
- 📋 **RK1399** (deps: RK1398) **The directory GitHub Pages serves is the governed store, so a build writing into it lands on the roadmap** — One directory holds both the files three verbs own and the files a browser fetches, and a build that mixes them is a deploy that can overwrite a task line. → §RK1399
- 📋 **RK1400** (deps: RK1399) **No job builds the area, so a page that stopped compiling is found by a reader and a stale commit is published** — The gate runs four checks on every push and none of them is this build, while the deploy is a branch, so what ships is whatever somebody last committed. → §RK1400
- 📋 **RK1401** (deps: —) **No read prints the parser as data, so anything outside a terminal retypes the verbs and their flags** — A reference typed by hand is a second declaration of a schema, wrong at the first renamed flag and silent about it, and help output is text written to be read once. → §RK1401
...
- 📋 **RK1407** (deps: RK1398) **The model the verbs act on is written for an agent mid-session and for whoever edits the code, never for a reader** — A line, a pointer, a block, a criterion and a decision are one system, and learning it one refusal at a time is how a tool gets used as six unrelated commands. → §RK1407
