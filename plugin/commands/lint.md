...
- Whitespace, a marker's codepoint, dep order, an annotation, a derived pointer, a dead queue
  entry, an empty criteria heading nothing answers: those are **derived**, and `python "${CLAUDE_PLUGIN_ROOT}/scripts/roadkeep.py" lint --fix` repairs them without touching a
